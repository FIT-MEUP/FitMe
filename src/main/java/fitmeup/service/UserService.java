...
public class UserService {
	...
	public String findUserEmail(String userName, String userContact) {
		return userRepository.findEmailByUserNameAndUserContact(userName, userContact).orElse("존재하지 않는 회원정보입니다.");
	}
	...
	public String verifyUserAndGenerateTempPassword(String userName, String userEmail, String userContact) {
		...
		// 📌 수정된 부분: 전화번호 변환 제거
		Optional<UserEntity> userOpt = userRepository.findByUserNameAndUserEmailAndUserContact(userName, userEmail,
				userContact);
	...
	public UserDTO findByEmail(String email) {
		return userRepository.findByUserEmail(email)
				.map(user -> UserDTO.builder().userId(user.getUserId()).userName(user.getUserName())
						.role(user.getRole().name()) // ✅ enum을 String으로 변환
						.build())
				.orElse(null); // ✅ null 반환 방지
	}

	/**
	 * 특정 userId가 'Approved' 상태인지 확인하는 메서드
	 * @param userId
	 * @return
	 */
	public boolean checkPt(Long userId) {
		return trainerApplicationRepository.existsByUserUserIdAndStatus(userId,
				TrainerApplicationEntity.Status.Approved);
	}
