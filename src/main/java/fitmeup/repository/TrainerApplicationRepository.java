...
public interface TrainerApplicationRepository extends JpaRepository<TrainerApplicationEntity, Long> {
	...
	// 특정 userId에 해당하는 TrainerApplicationEntity에서 연결된 TrainerEntity의 trainerId를 조회하는
	// 것
...
//  Long findTrainerIdByUserId(@Param("userId") Long userId);
	...
	boolean existsByUserUserIdAndTrainerTrainerIdAndStatus(Long userId, Long trainerId,
			TrainerApplicationEntity.Status status);
	...
	List<UserEntity> findApprovedUsersByTrainerId(@Param("trainerId") Long trainerId,
			@Param("status") TrainerApplicationEntity.Status status);
	...
	// 김윤성
	boolean existsByUserUserIdAndStatus(Long userId, TrainerApplicationEntity.Status status);
